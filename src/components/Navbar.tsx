...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center px-3 py-3 bg-gray-50 shadow sm:px-5 md:px-10 lg:px-24 fixed top-0 w-full'>
        ...
        <button onClick={openCart} className='relative cursor-pointer active:scale-105'>
            <img src={cartIcon} alt="" className='size-6'/>
            <span className='bg-red-700 text-white text-xs p-0.5 absolute -top-2 -right-1 rounded-3xl'>{cartQuantity}</span>
        </button>
    </div>
  )
}
